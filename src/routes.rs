...
use crate::key_pool::{CombinedKeyPool, SearchProvider};
...
use crate::providers::{
    ScrapeInput, ScrapeResponse, SearchAttemptError, SearchInput, SearchResponse,
    log_provider_failure, log_provider_success,
};
...
pub struct AppState {
    ...
    combined_search: Option<CombinedKeyPool>,
...
impl AppState {
    pub fn new(config: Config, client: reqwest::Client) -> Self {
        ...
        let combined_search = if matches!(config.search_upstream(), SearchUpstream::BraveTavily) {
            config
                .brave_keys()
                .zip(config.tavily_keys())
                .map(|(brave, tavily)| CombinedKeyPool::from_pools(brave, tavily))
        } else {
            None
        };
        Self {
...
async fn search(
    ...
    let response = tokio::time::timeout(REQUEST_TIMEOUT, async {
        match state.search_upstream {
            ...
            SearchUpstream::BraveTavily => search_combined(&state, &input).await,
        }
    })
    .await
...
async fn search_combined(
    ...
    if candidates.is_empty() {
        return Err(ApiError::NoEligibleKey { provider: "search" });
    }
    ...
    for (index, candidate) in candidates.iter().enumerate() {
        ...
        let result = match provider {
            SearchProvider::Brave => match state.brave.as_ref() {
                Some(provider) => provider.search_with_key(input, candidate.secret()).await,
                None => Err(SearchAttemptError::Api(ApiError::UpstreamUnavailable)),
            },
            SearchProvider::Tavily => match state.tavily.as_ref() {
                Some(provider) => provider.search_with_key(input, candidate.secret()).await,
                None => Err(SearchAttemptError::Api(ApiError::UpstreamUnavailable)),
            },
        };
        ...
        match result {
            Ok(response) => {
                ...
                log_provider_success(provider.as_str(), candidate.slot, attempt, started);
                return Ok(response);
            }
            Err(SearchAttemptError::Retryable(failure)) => {
                last_failure_class = failure.status_class();
                pool.mark_failure(candidate.slot, failure);
                log_provider_failure(provider.as_str(), candidate.slot, attempt, failure, started);
            }
            Err(SearchAttemptError::Api(error)) => return Err(error),
        }
    }
