...
#[derive(Clone)]
pub struct CombinedKeyCandidate {
    provider: SearchProvider,
    pub slot: usize,
    secret: String,
}

impl CombinedKeyCandidate {
...
impl fmt::Debug for CombinedKeyCandidate {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("CombinedKeyCandidate")
...
#[derive(Clone)]
pub struct CombinedKeyPool {
    inner: Arc<CombinedPoolInner>,
}

impl fmt::Debug for CombinedKeyPool {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("CombinedKeyPool")
            .field("providers", &"brave+tavily")
            .field("key_count", &self.inner.entries.len())
            .finish()
    }
}
...
impl KeyPool {
    ...
    pub fn mark_failure(&self, slot: usize, failure: FailureClass) {
        ...
        mark_failure_state(
            ...
            self.inner.keys.len(),
            failure,
            self.inner.initial_cooldown,
            self.inner.max_cooldown,
        );
    }
...
impl CombinedKeyPool {
    pub fn from_pools(brave: &KeyPool, tavily: &KeyPool) -> Self {
        Self::with_policy(
            brave,
            tavily,
            Duration::from_secs(30),
    ...
    pub fn with_policy(
        ...
        Self {
            inner: Arc::new(CombinedPoolInner {
                entries,
                state: Mutex::new(PoolState {
                    cursor: 0,
                    failure_streak: vec![0; len],
    ...
    pub fn candidates(&self) -> Vec<CombinedKeyCandidate> {
        ...
        for offset in 0..len {
            ...
            let available = state.cooldown_until[slot]
                ...
                .unwrap_or(true);
            if available {
                let entry = &self.inner.entries[slot];
                candidates.push(CombinedKeyCandidate {
    ...
    pub fn mark_success(&self, slot: usize) {
        if slot >= self.inner.entries.len() {
            return;
        }
...
fn mark_failure_state(
    ...
    if !failure.is_retryable() || slot >= len {
        return;
    }
