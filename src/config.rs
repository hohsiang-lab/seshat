...
impl FromStr for SearchUpstream {
    ...
    fn from_str(value: &str) -> Result<Self, Self::Err> {
        let normalized = value
            ...
            .collect::<Vec<_>>();
        let providers = normalized.iter().map(String::as_str).collect::<Vec<_>>();
        match providers.as_slice() {
            ["firecrawl"] => Ok(Self::Firecrawl),
            ["brave"] => Ok(Self::Brave),
            ["tavily"] => Ok(Self::Tavily),
            ["brave", "tavily"] | ["tavily", "brave"] => Ok(Self::BraveTavily),
...
impl Config {
    ...
    pub fn from_env_values(vars: &BTreeMap<String, String>) -> Result<Self, ConfigError> {
        ...
        let brave_keys = match search_upstream {
            SearchUpstream::Firecrawl | SearchUpstream::Tavily => None,
            SearchUpstream::Brave | SearchUpstream::BraveTavily => Some(KeyPool::from_file_or_env(
        ...
        let tavily_keys = match search_upstream {
            ...
            SearchUpstream::Tavily | SearchUpstream::BraveTavily => {
                Some(KeyPool::from_file_or_env(
