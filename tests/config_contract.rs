...
#[test]
fn combined_search_selector_accepts_both_orders_and_canonicalizes() {
    for selector in [
        "brave,tavily",
        "tavily,brave",
        " brave, tavily ",
        ...
        let config = Config::from_env_values(&vars(&[
            ("SESHAT_TOKEN", "auth"),
            ("SESHAT_SEARCH_UPSTREAM", selector),
            ("FIRECRAWL_API_KEYS", "firecrawl"),
            ("BRAVE_SEARCH_API_KEYS", "brave-a\nbrave-b\nbrave-c"),
            (
                "TAVILY_SEARCH_API_KEYS",
                "tavily-a\ntavily-b\ntavily-c\ntavily-d",
            ),
        ]))
        .expect("combined selector should load");

        assert_eq!(config.search_upstream(), SearchUpstream::BraveTavily);
        assert_eq!(config.search_upstream().as_str(), "brave+tavily");
        assert_eq!(config.brave_keys().expect("Brave pool").len(), 3);
        assert_eq!(config.tavily_keys().expect("Tavily pool").len(), 4);
        assert!(config.is_ready());
    }
}

#[test]
fn combined_search_selector_requires_both_provider_pools() {
    for key_name in ["BRAVE_SEARCH_API_KEYS", "TAVILY_SEARCH_API_KEYS"] {
        let mut values = vars(&[
            ("SESHAT_TOKEN", "auth"),
            ("SESHAT_SEARCH_UPSTREAM", "tavily,brave"),
            ("FIRECRAWL_API_KEYS", "firecrawl"),
            ("BRAVE_SEARCH_API_KEYS", "brave"),
            ("TAVILY_SEARCH_API_KEYS", "tavily"),
        ]);
        values.remove(key_name);

        let error = Config::from_env_values(&values)
            .expect_err("combined selector without either provider must fail");
        assert_eq!(error.code(), "missing_key_pool");
...
#[test]
fn combined_search_selector_rejects_duplicates_and_unsupported_provider_sets() {
    for selector in ["brave,brave", "brave,", ",tavily", "firecrawl,brave"] {
        let error = Config::from_env_values(&vars(&[
            ("SESHAT_TOKEN", "auth"),
            ("SESHAT_SEARCH_UPSTREAM", selector),
            ("FIRECRAWL_API_KEYS", "firecrawl"),
        ...
        .expect_err("unsupported provider set must fail");
        assert_eq!(error.code(), "invalid_configuration");
...
fn search_upstream_identifiers_are_stable() {
    ...
    assert_eq!(SearchUpstream::BraveTavily.as_str(), "brave+tavily");
}
